...
namespace E_APP02.SERVICES.SCIENCE_SERVICES.CHEMISTRY_SERVICES
{
    internal class Chemistry_Services
    {
        ...
        private static Sql_Services02 Sql_Serv01 = new Sql_Services02();
        ...
        private async Task load_find_Element_Textfile()
        {
            ...
            {
                data01[0] += Sql_Services02.insert_element(atomic_number[i],
                                                            element_name[i],
                                                            element_symboles[i],
                                                            atomic_mass[i],
                                                            protons[i],
        ...
        public string find_element_using_sql(string input)
        {
            ...
            if (Sql_Serv01.find_element_using_atomic_number(input) != Default_Text01.text02[2])
            {
                data01[4] = Sql_Serv01.find_element_using_atomic_number(input);
            }
            else if (Sql_Serv01.find_element_using_symbol(input) != Default_Text01.text02[2])
            {
                data01[4] = Sql_Serv01.find_element_using_symbol(input);
            }
            else if (Sql_Serv01.find_element_using_name(input) != Default_Text01.text02[2])
            {
                data01[4] = Sql_Serv01.find_element_using_name(input);
            }
            else
            {
        ...
        public async Task<string> find_category_elements_using_sql(int input)
        {
            ...
            if (input == 0)
            {
                data01[8] = await Sql_Servicces02.find_Alkali_Metals();
            }
            else if (input == 1)
            {
                data01[8] = await Sql_Servicces02.Find_Actinides();
            }
            else if (input == 2)
            {
                data01[8] = await Sql_Servicces02.find_Alkaline_Earth_Metals();
            }
            else if (input == 3)
            {
                data01[8] = await Sql_Servicces02.find_Lanthanides_Rare_Earth_Metals();
            }
            else if (input == 4)
            {
                data01[8] = await Sql_Servicces02.find_Noble_Gases();
            }
            else if (input == 5)
            {
                data01[8] = await Sql_Servicces02.find_Nonmetal_Gases_at_Room_Temperature();
            }
            else if (input == 6)
            {
                data01[8] = await Sql_Servicces02.find_Transition_Metals();
            }
            else
            {
                data01[8] = "IDK";
